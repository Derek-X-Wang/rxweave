...
**Last updated:** 2026-05-30 after v0.5.5 ship to npm + the OSS adoption cycle (rungs 1–3 + browsable example).
**Integration state:** 255 tests (11 packages, +1 skipped integration) passing locally. All 11 `@rxweave/*` packages live on npm at **v0.5.5**. **OSS adoption is validated end-to-end from pure npm**: a fresh `bun add @rxweave/cli@0.5.5` → `rxweave init --template full` → `rxweave dev` → `emit request.posted` → the `bob-assistant` agent reacts with a provenance-stamped `response.posted` → `inspect --ancestry` — proven by `scripts/smoke-quickstart.sh` run with `RXWEAVE_USE_NPM_DIST=1` against the published packages. A PR-based flow now gates `main` (CI runs typecheck+test+lint on every PR; repo auto-merge + branch protection requiring the check, with `enforce_admins:false` so direct tag-release pushes still work). Earlier browser-via-cloud work (v0.5.2→v0.5.4 + cloud-v0.3.5) remains valid. All releases since v0.5.1 are purely additive on the public surface — no wire-protocol change.
...
| `v0.5.3` | Token-mediated browser auth. `LiveFromBrowserOpts.token?: TokenProvider` — bearer-provider path for third-party browser consumers; when present, cookie bootstrap (`sessionTokenFetch`) is skipped. Provider wrapped in 5-min TTL cache, invalidated before each Subscribe attempt (initial + every watchdog reconnect) via the new internal `onSubscribeConnect` hook on `makeCloudEventStore`, so expiring Better Auth session tokens refresh on reconnect. Bug fix: `sessionTokenFetch` gained `credentials:"include"` (cross-origin cookie bootstrap was silently broken without it). Canonical pattern (Codex `/counsel`): `token` = primary for third-party browser; cookie bootstrap = same-origin convenience for hosted/OSS deploys. `+4` tests, 57/57 live conformance against cloud-v0.3.4. CI `26256160162`, 1 m 26 s. | 2026-05-21 |
| `v0.5.4` | WKWebView cross-origin streaming fix. `LiveFromBrowser` RPC fetches now carry `credentials:"include"` (both token + cookie paths) — WebKit refuses to deliver cross-origin `ReadableStream` body chunks without it even when the server sends `Access-Control-Allow-Credentials:true` + a specific `Access-Control-Allow-Origin`; Chrome unaffected. **Implementation gotcha (hard-won):** `FetchHttpClient.RequestInit` is read from `FiberRef.currentContext` at call time and is NOT propagated through `Layer.provide` composition — the naive `Layer.succeed(FetchHttpClient.RequestInit, …)` silently fails. Fix uses `HttpClient.transform` + `Effect.provide(send, credentialsCtx)` to inject `RequestInit` per-request. `+2` tests, 47/47 unit+auth green. Closed the browser-via-cloud end-to-end gap: cloud-v0.3.5 WKWebView smoke watchdog-silent 188 s. | 2026-05-23 |
| `v0.5.5` | **OSS adoption rung 1 + two pre-existing-bug fixes.** `rxweave init --template full` scaffolds a runnable "collaboration stream skeleton" (`alice` posts `request.posted` via the CLI → `bob-assistant` stub agent emits `response.posted` on the shared log, auto-stamped `actor`/`source`/`causedBy`); the `--template full` flag was advertised but inert before. README quickstart rewritten to use it. **Fixes the smoke surfaced:** (1) `@rxweave/cli` `buildFilter` set an empty-array filter from an absent repeated option (`@effect/cli` yields `Some([])`, not `None`) — rejected *every* event; (2) `@rxweave/store-file` `subscribe` only saw same-process appends — a byte-exact file-tail polling fiber now surfaces cross-process events (required by the two-process `dev &` + `emit` quickstart); (3) a **Critical silent data-loss bug** in that tail (cold-start recovery offset in UTF-16 units vs the tail's byte offset → a multi-byte-UTF-8 recovered file dropped the next appended event) — recovery + tail now share one byte-exact `scanLines`. `@rxweave/cli` 36→52 tests, `@rxweave/store-file` 16→20. Released direct-to-`main` + tag (admin-bypass); the post-publish smoke step was hardened to wait for npm propagation via `npm view` before its single install gate (was false-failing on propagation lag). | 2026-05-30 |

**v0.5.5 is the current npm install target** (verified: a full pure-npm quickstart round-trip via `scripts/smoke-quickstart.sh` with `RXWEAVE_USE_NPM_DIST=1`). v0.5.5 is the first version where the documented quickstart actually works end-to-end for an outside consumer — v0.5.4 and earlier had the empty-filter + cross-process-visibility bugs that the adoption smoke caught. All prior versions v0.1.0–v0.4.0 shipped with `workspace:^` in their `dependencies` (broken on `bun add @rxweave/cli` in a fresh dir). Per-user note from earlier: `npm deprecate` of v0.1.0–v0.5.4 is intentionally deferred — solo author, no external users today.
...
## Sub-project status (all at v0.5.5 — npm + workspace)
...
| `@rxweave/store-cloud` | 18 unit/auth + 1 skipped integration (+ shared conformance) | `CloudStore.Live` + `CloudStore.LiveFromBrowser({ origin, token?, tokenPath?, heartbeat? })`: session-token bootstrap **or** token-provider bearer path (v0.5.3), QueryAfter drain, heartbeat sentinel (15s default), reconnect-from-lastDelivered. v0.5.2: ~134-line watchdog dance collapsed to `heartbeatGuard(intervalMs)`. v0.5.3: `token` provider (5-min TTL cache, invalidated per-reconnect via `onSubscribeConnect`) skips cookie bootstrap; `sessionTokenFetch` gained `credentials:"include"`. v0.5.4: all browser RPC fetches carry `credentials:"include"` via `HttpClient.transform` + per-request `Effect.provide` (Layer-level injection silently fails — `FetchHttpClient.RequestInit` reads from `FiberRef.currentContext` at call time). `WatchdogTimeout` re-exported for back-compat; `CloudRpcClient.Subscribe` type honest `Stream<EventEnvelope \| Heartbeat, …>`. |
...
**The OSS adoption cycle is complete** (direction set 2026-05-23 via Codex `/counsel`: for a zero-external-user project the limiting constraint is *adoption*, not cloud internals — Codex pushed back on chasing "native streaming," which optimizes for subscribers that don't exist). Shipped across PRs #10–#15 on 2026-05-30. Spec + plan: `docs/superpowers/specs|plans/2026-05-30-oss-adoption-quickstart*`.

1. **OSS adoption — done.**
   - **Rung 1** — `rxweave init --template full` scaffolds the collaboration-stream skeleton (`alice` posts `request.posted` → `bob-assistant` stub agent emits `response.posted` on the shared log, auto-stamped provenance). Shipped in **v0.5.5**, validated end-to-end from pure npm.
   - **Rung 2** — `docs/cookbook/llm-agent.md`: swap the stub for a real Claude agent (`@rxweave/llm`'s `defineLlmAgent`). Linked from the README cookbook list.
   - **Rung 3** — documented (a second human + their agent on the same stream; the "paste this prompt into your Claude" relay gone — the project's whole point).
   - **Browsable example** — `examples/collaboration-stream/` (the verbatim `--template full` output + a `package.json`), guarded against drift from the template by a `@rxweave/cli` test (`examples.test.ts`). Not a workspace member; installs `@rxweave/*` from npm like a real consumer.
   - **Bug fixes the adoption smoke surfaced** (now all on npm in v0.5.5): cli empty-filter; store-file cross-process file-tail; a Critical silent data-loss bug (char-vs-byte offset desync) — recovery + tail now share one byte-exact `scanLines`.
   - **Infra added this cycle:** PR CI (`.github/workflows/ci.yml` — typecheck+test+lint on every PR), repo auto-merge + branch protection (`enforce_admins:false` keeps direct tag-release pushes working), and a release-smoke fix (wait for npm propagation via `npm view`, then a single install gate — was false-failing releases on propagation lag).

### Next direction (reassess before defaulting to cloud internals)

**Caveat to carry in:** the items below are Codex's post-adoption sequence, but all three serve/optimize *cloud internals for users who don't exist yet* — the same "zero-user" critique that redirected the last cycle to adoption still applies. Adoption is now landed. The highest-leverage next move may instead be non-code (get RxWeave in front of people) or a new OSS-toolkit capability. Weigh that before picking up the cloud work below.

2. **Small server hardening** — `max-events-per-batch` cap on Append (prevent a 100k-event append locking a mutation) + max-registry-size cap. Bounded, Convex-native, touches the Append loop in `cloud/packages/backend/convex/rxweaveRpcBypass.ts`. Robustness depth, not an open hole (the endpoint is auth-gated). Cloud is local-only — do it with direct commits, not the PR flow.

3. **Dashboard analytics** — event-count/hour histograms, agent throughput, lineage graph, schema-inspection panel. Worth doing once there's real data to inspect.

4. **Native streaming spike** — *last*, or sooner only if the reference app makes the 1 s Subscribe latency visibly embarrassing. Spike-before-build (Codex-confirmed). Sharpest feasibility question: *can a Convex httpAction hold a response open and push a chunk immediately when Convex data changes — without polling `queryEventsAfter` — while preserving the NDJSON Subscribe wire contract?* Bar: chunk arrives < ~250 ms after a separate Append, no idle poll loop, stream survives several minutes, client reconnect resumes from last cursor. Fail → document the wall, keep polling. If it needs SSE/WebSocket + a transport-aware `@rxweave/store-cloud` client, it's a transport redesign, not a Subscribe optimization.

5. **`npm deprecate` of v0.1.0–v0.5.4** — solo-author note: skip until external users exist. v0.5.0 specifically breaks for external bundlers (no `@rxweave/core` runtime dep) — fixed in v0.5.1; deprecate when an external user reports.
