...
**Last updated:** 2026-05-23 after v0.5.4 ship to npm + cloud-v0.3.5 (token-mediated browser auth validated end-to-end).
**Integration state:** 251 tests (11 packages) + 12 conformance passing locally. All 11 `@rxweave/*` packages live on npm at **v0.5.4** (Trusted Publishing via tag push; verified `bun add @rxweave/cli@0.5.4` in a fresh `/tmp` dir → `bunx rxweave --version` → `0.5.4`). **Browser-via-cloud is now validated end-to-end**: a real WKWebView (cmux) running `apps/web` pointed at the live cloud-v0.3.5 deployment authenticates via the token-mediated `LiveFromBrowser({ origin, token })` path, opens Subscribe, and stays watchdog-silent for 188 s. The v0.5.2→v0.5.4 arc closed the cross-repo browser auth story (see "What shipped" v0.5.3/v0.5.4 rows + cloud HANDOFF). All releases since v0.5.1 are purely additive on the public surface — no wire-protocol change.
...
**v0.5.4 is the current npm install target** (verified `bun add @rxweave/cli@0.5.4` in a fresh `/tmp` dir → `bunx rxweave --version` → `0.5.4`). v0.5.1–v0.5.3 are functionally usable for anyone pinned to them — every diff since v0.5.1 is purely additive (browser-auth ergonomics + WebKit compat) — but v0.5.4 is what new consumers should install. All prior versions v0.1.0–v0.4.0 shipped with `workspace:^` in their `dependencies` (broken on `bun add @rxweave/cli` in a fresh dir). Per-user note from earlier: `npm deprecate` of v0.1.0–v0.5.3 is intentionally deferred — solo author, no external users today.
...
## Sub-project status (all at v0.5.4 — npm + workspace)
...
2. **Small server hardening** — `max-events-per-batch` cap on Append (prevent a 100k-event append locking a mutation) + max-registry-size cap. Bounded, Convex-native, touches the Append loop in `cloud/packages/backend/convex/rxweaveRpcBypass.ts`. Defer heavier per-tenant rate limiting until the endpoint is publicly exposed (today it's auth-gated, so this is robustness depth not an open hole).

3. **Dashboard analytics** — event-count/hour histograms, agent throughput, lineage graph, schema-inspection panel. Worth doing once the adoption path produces data worth inspecting.

4. **Native streaming spike** — *last*, or sooner only if the reference app makes the 1 s Subscribe latency visibly embarrassing. Spike-before-build (confirmed by Codex). Sharpest feasibility question: *can a Convex httpAction hold a response open and push a chunk immediately when Convex data changes — without polling `queryEventsAfter` — while preserving the NDJSON Subscribe wire contract?* Bar: chunk arrives < ~250 ms after a separate Append, no idle poll loop, stream survives several minutes, client reconnect resumes from last cursor. Fail → document the wall, keep polling. If it needs SSE/WebSocket + a transport-aware `@rxweave/store-cloud` client, it's a transport redesign, not a Subscribe optimization.

5. **`npm deprecate` of v0.1.0–v0.5.3** — solo-author note: skip until external users exist. v0.5.0 specifically breaks for external bundlers (no `@rxweave/core` runtime dep) — fixed in v0.5.1; deprecate when an external user reports.
