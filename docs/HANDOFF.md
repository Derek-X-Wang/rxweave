...
**Last updated:** 2026-05-15 after v0.5.2 local-tagged (awaiting push approval).
**Integration state:** 251 tests (11 packages) + 12 conformance passing. All 11 `@rxweave/*` packages bumped to v0.5.2 locally; **not yet on npm** — push (and the Trusted Publishing CI run that follows) is gated behind user authorization. Current npm install target stays **v0.5.1** until that push lands. Cross-repo browser-streaming end-to-end verification from v0.5.1 (cadence probe 9 heartbeats / 10 s @ `intervalMs=1000`, `@rxweave/store-cloud` conformance 52/52 green against live cloud at `https://giant-caribou-833.convex.site`) still applies — v0.5.2 is purely additive on the public surface plus a sub-second-interval watchdog bug fix; no wire-protocol change. v0.5.0's manual Safari smoke through cmux WKWebView remains valid for the rxweave-side path.
...
| `v0.5.2` *(local-tagged, awaiting push)* | Heartbeat-contract consolidation. New `@rxweave/protocol/Heartbeat.ts` module owns the wire sentinel + watchdog contract end-to-end: `clampIntervalMs`, `makeHeartbeatStream` (self-clamping internally so a future private cloud handler bypassing `@effect/rpc` cannot re-introduce the drift), `heartbeatGuard` Stream combinator (hides the `lastHeartbeatAt` Ref + 1Hz polling watchdog), `isHeartbeat` predicate, `WatchdogTimeout` (moved from `@rxweave/store-cloud/Errors.ts`, re-exported for backward compat). Subscribe handler trims to ~5 lines around the new primitives. `CloudStore.ts` watchdog block (~134 lines) collapses to a single `heartbeatGuard(intervalMs)` call. **Two bug fixes**: (1) sub-second `intervalMs` requests no longer false-fire the watchdog (pre-v0.5.2 client threshold used unclamped value while server clamped up to 1000 ms — sub-second requests triggered ~1 s reconnect storms); (2) `CloudRpcClient.Subscribe` type honesty repaired (was `Stream<EventEnvelope, …>`, now `Stream<EventEnvelope \| Heartbeat, …>` matching the wire schema). Codex second-opinion via the `/counsel` skill caught the self-clamping requirement and the type-lie before commit. Purely additive on the public surface, no wire-protocol change. | 2026-05-15 |

**v0.5.1 remains the current npm install target until v0.5.2 push lands.** Once the user authorizes `git push origin main v0.5.2`, Trusted Publishing via the existing release workflow takes over and v0.5.2 becomes the live target. Verification step (post-push): `bun add @rxweave/cli@0.5.2` in a fresh `/tmp` dir; `bunx rxweave --version` → `0.5.2`. All prior versions v0.1.0–v0.4.0 shipped with `workspace:^` in their `dependencies` (broken on `bun add @rxweave/cli` in a fresh dir). Per-user note from earlier: `npm deprecate` of v0.1.0–v0.5.1 is intentionally deferred — solo author, no external users today.
...
## Sub-project status (all at v0.5.2 in workspace; v0.5.1 on npm pending push)
...
1. **`git push origin main v0.5.2`** — local commit + tag complete (this session). Push is user-authorized: the existing release workflow handles Trusted Publishing on the tag and the bumped 10×15 s post-publish smoke retry validated by v0.5.1's CI run. After push, verify `bun add @rxweave/cli@0.5.2` in a fresh `/tmp` dir → `bunx rxweave --version` → `0.5.2`. Update HANDOFF's "Last updated" line + "current npm install target" once verified.

2. **Cross-repo: team-cloud adopts v0.5.2 heartbeat primitives.** Tracked as task #2 in the project task list. After v0.5.2 hits npm, the private cloud-v0.3 hand-rolled Subscribe handler (`cloud/packages/backend/convex/rxweaveSubscribeBypass.ts` and `rxweaveRpc.ts`, owned by team-cloud) can import `clampIntervalMs` + `makeHeartbeatStream` from `@rxweave/protocol` and drop its own clamp logic. Eliminates the same clamp drift on the Convex side. Lead will dispatch when ready; no blocking dep on this release.
...
4. **`npm deprecate` of v0.1.0–v0.5.0** — solo-author note: skip until external users exist. v0.5.0 specifically breaks for external bundlers (no `@rxweave/core` runtime dep) — fixed in v0.5.1; deprecate when an external user reports.

## Shipped in v0.5.2 (local-tagged, awaiting push)
